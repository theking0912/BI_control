...
sql_string = "select c.table_id, c.table_name, c.ext_system, c.run_lastdate, c.run_status, c.load_table_name" \
             ...
             "     inner join BIetluser.Bi_Init_Export d " \
...
for table in tables:
    ...
    if run_lastdate == now_date and run_status == 1:
        ...
        control_file = "OPTIONS (skip=1) \n" \
                       "load data \n" \
                       "infile '" + CONTROL_FILE_PATH + "data/" + YESTODAY + "/init_data/" + table_name + "_" + YESTODAY + "_init.csv'\n" \
...
Sp_str = "BIetluser.p_del_tw_repeat_data"
