...
" from bi_etl_table " \
" where is_valid =1 " \
" and table_name in ('VBAK','VBAP')"
# and table_name in ('KNA1','KNVV','MAKT','MARA','MARM','TVKOT','VBAK','VBAP')
db=connectDB()
...
for table in tables:
    ...
    if run_schedule=='DAY':
        ...
        ext_data_file2 = ext_data_file.replace('{#file_name}', table_name).replace('{#yesterday}', ext_sql_param_value)
